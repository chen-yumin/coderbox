...
describe('ThemeService', () => {
  ...
  beforeEach(() => {
    ...
    overlayClassList = document.body
      .getElementsByClassName('cdk-overlay-container')[0].classList;
  });

  it('should initialize with default theme', () => {
    expect(service).toBeTruthy();
    expect(service.darkTheme).toBe(service.themeMatcher.matches,
  ...
  it('should add "dark-theme" class after setting darkTheme to `true`', () => {
    ...
    expect(bodyClassList.contains('light-theme')).toBe(false,
      'should not contain "light-theme" class');
    expect(overlayClassList.contains('light-theme')).toBe(false,
      'should not contain "light-theme" class');
    expect(bodyClassList.contains('dark-theme')).toBe(true,
      'should contain "dark-theme" class');
    expect(overlayClassList.contains('dark-theme')).toBe(true,
      'should contain "dark-theme" class');
  });

  it('should add "light-theme" class after setting darkTheme to `false`', () => {
    service.darkTheme = false;
    expect(bodyClassList.contains('light-theme')).toBe(true,
      'should contain "light-theme" class');
    expect(overlayClassList.contains('light-theme')).toBe(true,
      'should contain "light-theme" class');
    expect(bodyClassList.contains('dark-theme')).toBe(false,
      'should not contain "dark-theme" class');
    expect(overlayClassList.contains('dark-theme')).toBe(false,
      'should not contain "dark-theme" class');
  });
